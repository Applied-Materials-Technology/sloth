                                                       ...
                                                       const Real & scalar)
{
  ...
  if (_yield_condition > 0.0)
  {
    ...
    const Real xflow =  (effective_trial_stress - (_three_shear_modulus * scalar) -
                                  computeHardeningValue(scalar) - _yield_stress)/_eta;
    const Real xphi = std::pow(xflow,_n);

    _xphidp = ((-_three_shear_modulus * _n)/_eta)*std::pow(xflow,_n-1);
    _xphir = (-_n/_eta) * std::pow(xflow,_n-1);
